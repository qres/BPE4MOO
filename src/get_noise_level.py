...
def get_noise_level(
    obj_func, input_dim, comp_noise_type, target_error, top_proportion, num_samples, seed=0
):
    prng = torch.Generator().manual_seed(seed)

    X = torch.rand([num_samples, input_dim], generator=prng)
    Y = obj_func(X)
    target_Y = Y.sort().values[-int(num_samples * top_proportion) :]
    target_Y = target_Y[torch.randperm(target_Y.shape[0], generator=prng)]
...
def error_rate_loss(x, obj_vals, true_comps, target_error, comp_noise_type):
    ...
    )**2 # square for a smooth optimization problem
